...
ggs = fig.add_gridspec(2, 6, wspace=0.9)

f1 = fig.add_subfigure(ggs[1, 0:2])
...
f1 = fig.add_subfigure(ggs[1, 4:6])
gs = f1.add_gridspec(1, 1)
gs.update(top=0.8, bottom=0, left=0, right=0.9)
a6 = f1.add_subplot(gs[0, 0])
plot_scatter(a6, -2)
a6.text(-0.05, 1.01, "F", transform=a6.transAxes, size=30, weight='bold')
...
subfig_regression_line(f1, bg_absorptions, (bg_signals - intercept) / slope,
                       ylabel="Estimated absorption", title="Cal. signal (background)", color=COLOURS[3], num="A", right=0.8, top=0.9,
                       bottom=0.1)
subfig_regression_line(f2, inc_absorptions, (inc_signals - intercept) / slope,
                       ylabel="Estimated absorption", title="Cal. signal (inclusion)", color=COLOURS[3], num="B",  right=0.8, top=0.9,
                       bottom=0.1, inclusions=True, first=False)


f1 = fig.add_subfigure(ggs[0, 4:6])
gs = f1.add_gridspec(1, 1)
gs.update(top=0.9, bottom=0.1, left=0, right=0.9)
a4 = f1.add_subplot(gs[0, 0])
a4.set_title("Simulation vs experiment")
...
a4.plot(depth_levels * SPACING, depth_sim_mean, "--", color=COLOURS[0], label="MC model")
...
a4.vlines([depth_levels[idx] * SPACING for idx in [1, 8, -2]], 0, 1.2, "black", linestyles="--")
a4.text(depth_levels[1] * SPACING + 0.1, 0.4, "D")
a4.text(depth_levels[8] * SPACING + 0.1, 0.4, "E")
a4.text(depth_levels[-2] * SPACING + 0.1, 0.4, "F")
...
a4.text(-0.2, 1.01, "C", transform=a4.transAxes, size=30, weight='bold')



#plt.subplots_adjust(wspace=0, hspace=0)
plt.savefig(r"..\figures\depth_correlation.png", bbox_inches='tight')
plt.savefig(r"..\figures\depth_correlation.svg", bbox_inches='tight')
plt.savefig(r"..\figures\depth_correlation.pdf", bbox_inches='tight')
plt.close()